...
const path = (
  ...
  if (route === undefined) throw new Error(`No route found for: ${routeName}`);
...
const routeHaveParams = (route: string): boolean => REGEX_PARAM_ROUTER.test(route);
...
const replaceParams = (
  ...
  if (paramMustBeReplace) {
    params.forEach((param: string): void => {
      ...
      if (option === undefined) throw new Error(`Missing parameter: ${param}`);
...
const hasRestOptions = (
  options: { [key: string]: string },
  params: Array<string>,
): boolean => Object.keys(options).filter((option: string) => !params.includes(option)).length > 0;
